  ...
  useEffect(() => {
    if (user && currentStep === 0) {
      ...
      setFormData(prev => ({
        ...
        apellidos: user.displayName?.split(' ').slice(1).join(' ') || prev.apellidos,
      }));
    }
  }, [user, currentStep]);
    ...
    <div style={{padding: '100px 5%', minHeight: '100vh', background: '#f5f5f5', color: '#333', fontFamily: 'Montserrat, sans-serif'}}>
      <div style={{maxWidth: '1200px', margin: '0 auto', display: 'flex', flexWrap: 'wrap', gap: '40px'}}>
        ...
        <div style={{flex: '1 1 600px'}}>
          ...
          {!user && (
            <div style={stepContainerStyle(currentStep === 0)}>
              <div style={stepHeaderStyle} onClick={() => setCurrentStep(0)}>
                <h3 style={{fontSize: '1.2rem', margin: 0, textTransform: 'uppercase', letterSpacing: '1px'}}>0. IDENTIFICACIÓN</h3>
              </div>
              
              {currentStep === 0 && (
                <div style={stepBodyStyle}>
                  <p style={{marginBottom: '20px', color: '#666', fontSize: '0.95rem', lineHeight: '1.5'}}>
                    Para un proceso de compra más rápido y hacer seguimiento de tus pedidos, te recomendamos iniciar sesión.
                  </p>
                  ...
                  <div style={{display: 'flex', flexDirection: 'column', gap: '15px', maxWidth: '350px'}}>
                    <button 
                      ...
                      style={{...btnDarkStyle, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', background: '#4285F4'}}
                    >
                    ...
                    <div style={{textAlign: 'center', color: '#999', fontSize: '0.9rem', margin: '10px 0'}}>o</div>
                    
                    <button 
                      onClick={() => setCurrentStep(1)} 
                      style={{...btnDarkStyle, background: '#eee', color: '#333', border: '1px solid #ddd'}}
                    >
